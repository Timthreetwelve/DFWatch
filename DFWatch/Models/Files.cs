...
internal static class Files
{
    ...
    {
        NLogHelpers.Log.Debug($"Checking for locks on {file.Name}.");
        ...
        for (int i = 0; i < UserSettings.Setting.NumRetries; i++)
        {
            try
            {
                ...
                if (i == 0)
                {
                    NLogHelpers.Log.Debug($"No locks detected on file {file.Name}.");
                }
                else
                {
                    NLogHelpers.Log.Info($"Obtained exclusive lock on file {file.Name} on attempt {i + 1}.");
                }
                return true;
            }
            catch (IOException ex)
            {
                ...
                {
                    NLogHelpers.Log.Warn($"{file.Name} cannot be moved. It is being used by another process.");
                    NLogHelpers.Log.Debug($"Waiting {UserSettings.Setting.RetryDelay} milliseconds before trying again.");
                    await Task.Delay(UserSettings.Setting.RetryDelay);
                }
                else if (ex.Message.Contains("Could not find file"))
                {
                    NLogHelpers.Log.Warn($"{file.Name} was not found.");
                    return false;
                }
                else
                {
                    NLogHelpers.Log.Warn($"{ex.Message}");
                    NLogHelpers.Log.Debug($"Waiting {UserSettings.Setting.RetryDelay} milliseconds before trying again.");
                    await Task.Delay(UserSettings.Setting.RetryDelay);
                }
            }
        ...
        NLogHelpers.Log.Warn($"Failed to get exclusive lock on {file} after {UserSettings.Setting.NumRetries} attempts.");
        return false;
    }
    ...
    public static async void MoveFile(FileInfo file)
    {
        ...
        string destinationFile = Path.Combine(UserSettings.Setting.DestinationFolder, file.Name);

        // Rename if destination file already exists
        if (File.Exists(destinationFile) && UserSettings.Setting.RenameIfDuplicate)
        {
            NLogHelpers.Log.Debug($"{destinationFile} already exists. Will attempt to rename.");
        ...
        {
            NLogHelpers.Log.Warn($"{destinationFile} already exists. Option to rename is false.");
            return;
        }
        ...
        if (await CheckForLocksAsync(file))
        {
            try
            {
                ...
                NLogHelpers.Log.Info($"Moved {file.Name} to {destinationFile}.");
            }
            catch (IOException) when (File.Exists(destinationFile))
            {
                NLogHelpers.Log.Error($"Failed to move {file.Name} because {destinationFile} already exists.");
            }
            catch (Exception ex)
            {
                NLogHelpers.Log.Error(ex, $"Failed to move {file.Name} to {destinationFile}.");
            }
        }
    }
    ...
    public static string CreateUniqueFileName(string filePath)
    {
        if (File.Exists(filePath))
        {
            ...
            NLogHelpers.Log.Debug($"{fileName}{fileExtension} will be renamed to {newFileName}");
        }
        return filePath;
    }
